...
/// <summary>
/// Editor script to wire the OrcWarBoss FBX model into the OrcWarBoss EnemyData asset.
/// Creates an OrcWarBossAnimator controller with Walk/Attack/Die states,
/// then assigns modelPrefab + animatorController on OrcWarBoss.asset.
/// Run via: Tools > Wire OrcWarBoss Model
/// </summary>
public class OrcWarBossWiring
{
    [MenuItem("Tools/Wire OrcWarBoss Model")]
    public static void WireOrcWarBossModel()
    {
        string fbxPath = "Assets/Models/OrcWarBoss.fbx";
        string controllerPath = "Assets/Animations/OrcWarBossAnimator.controller";
        string enemyDataPath = "Assets/ScriptableObjects/Enemies/OrcWarBoss.asset";

        // ── 1. Load FBX ──────────────────────────────
        var fbxAsset = AssetDatabase.LoadAssetAtPath<GameObject>(fbxPath);
        if (fbxAsset == null)
        {
            Debug.LogError($"[OrcWarBossWiring] FBX not found at {fbxPath}. Export from Blender first.");
            return;
        }
        Debug.Log($"[OrcWarBossWiring] Loaded FBX: {fbxPath}");
        ...
        foreach (var asset in allAssets)
        {
            if (asset is AnimationClip clip && !clip.name.StartsWith("__"))
            {
                ...
                Debug.Log($"[OrcWarBossWiring] Found clip: {clip.name} (length={clip.length:F2}s)");
        ...
        {
            Debug.LogError($"[OrcWarBossWiring] Missing clips! Walk={walkClip != null}, Attack={attackClip != null}, Die={dieClip != null}");
            Debug.Log("[OrcWarBossWiring] Available clips in FBX:");
            foreach (var asset in allAssets)
            {
        ...
        if (importer != null)
        {
            ...
            foreach (var clipAnim in clipAnimations)
            {
                if (clipAnim.name.ToLower().Contains("walk") && !clipAnim.loopTime)
                {
                    ...
                    changed = true;
                    Debug.Log($"[OrcWarBossWiring] Set {clipAnim.name} to loop");
                }
            }
        ...
        Debug.Log($"[OrcWarBossWiring] AnimatorController created at {controllerPath} with Walk, Attack, Die states");

        // ── 5. Wire OrcWarBoss.asset ScriptableObject ────
        var enemyData = AssetDatabase.LoadAssetAtPath<EnemyData>(enemyDataPath);
        if (enemyData == null)
        {
            Debug.LogError($"[OrcWarBossWiring] EnemyData not found at {enemyDataPath}");
            return;
        }
        ...
        Debug.Log($"[OrcWarBossWiring] OrcWarBoss.asset wired: modelPrefab={fbxAsset.name}, animatorController={controller.name}");
        Debug.Log("[OrcWarBossWiring] Done! Orc War Boss enemies will now use the custom model at runtime.");
    }
}
