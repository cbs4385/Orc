...
/// with baked octagonal towers. Square layout with 4 walls per side (east side
/// skips 1 slot for gate). Uses WALL_SPACING from the model geometry.
/// </summary>
public static class FortressBuilder
{
    ...
    public static void RebuildFortress()
    {
        ...
        // Reposition the existing Gate_E at slot 2 on the east side
        // Gate (2.0 wide) replaces one wall slot (2.2072 wide) — ~0.1 unit gap each side, covered by tower overlap
        var gate = wallManager.GetComponentInChildren<Gate>(true);
        if (gate != null)
        {
            gate.transform.localPosition = new Vector3(SIDE_OFFSET, 0f, SLOTS[2]);
            gate.transform.localRotation = Quaternion.Euler(0, 90, 0);
            gate.gameObject.name = "Gate_E";
            Debug.Log($"[FortressBuilder] Repositioned Gate_E to ({SIDE_OFFSET}, 0, {SLOTS[2]}) rot Y=90.");
        }
        else
        {
            Debug.LogWarning("[FortressBuilder] No Gate found under WallManager! Gate gap will be empty.");
        }
        ...
        // East side (x = +SIDE_OFFSET, Y rotation = 90): skip slot 2 for gate
        // Gate at SLOTS[2] — 3 walls + gate on east side
        count += PlaceWall(wallPrefab, wallManager.transform,
            new Vector3(SIDE_OFFSET, WALL_Y, SLOTS[0]), 90f, "Wall_E_0");
        count += PlaceWall(wallPrefab, wallManager.transform,
            new Vector3(SIDE_OFFSET, WALL_Y, SLOTS[1]), 90f, "Wall_E_1");
        // SLOTS[2] = gate position, skip
        count += PlaceWall(wallPrefab, wallManager.transform,
            new Vector3(SIDE_OFFSET, WALL_Y, SLOTS[3]), 90f, "Wall_E_3");

        Debug.Log($"[FortressBuilder] Placed {count} walls (15 expected). Fortress rebuilt.");
