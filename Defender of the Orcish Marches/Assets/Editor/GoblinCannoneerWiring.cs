...
/// <summary>
/// Editor script to wire the GoblinCannoneer FBX model into the GoblinCannoneer EnemyData asset.
/// Creates a GoblinCannoneerAnimator controller with Walk/Attack/Die states,
/// then assigns modelPrefab + animatorController on GoblinCannoneer.asset.
/// Run via: Tools > Wire GoblinCannoneer Model
/// </summary>
public class GoblinCannoneerWiring
{
    [MenuItem("Tools/Wire GoblinCannoneer Model")]
    public static void WireGoblinCannoneerModel()
    {
        string fbxPath = "Assets/Models/GoblinCannoneer.fbx";
        string controllerPath = "Assets/Animations/GoblinCannoneerAnimator.controller";
        string enemyDataPath = "Assets/ScriptableObjects/Enemies/GoblinCannoneer.asset";

        // ── 1. Load FBX ──────────────────────────────
        var fbxAsset = AssetDatabase.LoadAssetAtPath<GameObject>(fbxPath);
        if (fbxAsset == null)
        {
            Debug.LogError($"[GoblinCannoneerWiring] FBX not found at {fbxPath}. Export from Blender first.");
            return;
        }
        Debug.Log($"[GoblinCannoneerWiring] Loaded FBX: {fbxPath}");
        ...
        foreach (var asset in allAssets)
        {
            if (asset is AnimationClip clip && !clip.name.StartsWith("__"))
            {
                ...
                Debug.Log($"[GoblinCannoneerWiring] Found clip: {clip.name} (length={clip.length:F2}s)");
        ...
        {
            Debug.LogError($"[GoblinCannoneerWiring] Missing clips! Walk={walkClip != null}, Attack={attackClip != null}, Die={dieClip != null}");
            Debug.Log("[GoblinCannoneerWiring] Available clips in FBX:");
            foreach (var asset in allAssets)
            {
        ...
        if (importer != null)
        {
            ...
            foreach (var clipAnim in clipAnimations)
            {
                if (clipAnim.name.ToLower().Contains("walk") && !clipAnim.loopTime)
                {
                    ...
                    changed = true;
                    Debug.Log($"[GoblinCannoneerWiring] Set {clipAnim.name} to loop");
                }
            }
        ...
        Debug.Log($"[GoblinCannoneerWiring] AnimatorController created at {controllerPath} with Walk, Attack, Die states");

        // ── 5. Wire GoblinCannoneer.asset ScriptableObject ────
        var enemyData = AssetDatabase.LoadAssetAtPath<EnemyData>(enemyDataPath);
        if (enemyData == null)
        {
            Debug.LogError($"[GoblinCannoneerWiring] EnemyData not found at {enemyDataPath}");
            return;
        }
        ...
        Debug.Log($"[GoblinCannoneerWiring] GoblinCannoneer.asset wired: modelPrefab={fbxAsset.name}, animatorController={controller.name}");
        Debug.Log("[GoblinCannoneerWiring] Done! Goblin Cannoneer enemies will now use the custom model at runtime.");
    }
}
