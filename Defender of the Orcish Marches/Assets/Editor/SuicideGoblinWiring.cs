...
/// <summary>
/// Editor script to wire the SuicideGoblin FBX model into the SuicideGoblin EnemyData asset.
/// Creates a SuicideGoblinAnimator controller with Walk/Attack/Die states,
/// then assigns modelPrefab + animatorController on SuicideGoblin.asset.
/// Run via: Tools > Wire SuicideGoblin Model
/// </summary>
public class SuicideGoblinWiring
{
    [MenuItem("Tools/Wire SuicideGoblin Model")]
    public static void WireSuicideGoblinModel()
    {
        string fbxPath = "Assets/Models/SuicideGoblin.fbx";
        string controllerPath = "Assets/Animations/SuicideGoblinAnimator.controller";
        string enemyDataPath = "Assets/ScriptableObjects/Enemies/SuicideGoblin.asset";

        // ── 1. Load FBX ──────────────────────────────
        var fbxAsset = AssetDatabase.LoadAssetAtPath<GameObject>(fbxPath);
        if (fbxAsset == null)
        {
            Debug.LogError($"[SuicideGoblinWiring] FBX not found at {fbxPath}. Export from Blender first.");
            return;
        }
        Debug.Log($"[SuicideGoblinWiring] Loaded FBX: {fbxPath}");
        ...
        foreach (var asset in allAssets)
        {
            if (asset is AnimationClip clip && !clip.name.StartsWith("__"))
            {
                ...
                Debug.Log($"[SuicideGoblinWiring] Found clip: {clip.name} (length={clip.length:F2}s)");
        ...
        {
            Debug.LogError($"[SuicideGoblinWiring] Missing clips! Walk={walkClip != null}, Attack={attackClip != null}, Die={dieClip != null}");
            Debug.Log("[SuicideGoblinWiring] Available clips in FBX:");
            foreach (var asset in allAssets)
            {
        ...
        if (importer != null)
        {
            ...
            foreach (var clipAnim in clipAnimations)
            {
                if (clipAnim.name.ToLower().Contains("walk") && !clipAnim.loopTime)
                {
                    ...
                    changed = true;
                    Debug.Log($"[SuicideGoblinWiring] Set {clipAnim.name} to loop");
                }
            }
        ...
        Debug.Log($"[SuicideGoblinWiring] AnimatorController created at {controllerPath} with Walk, Attack, Die states");

        // ── 5. Wire SuicideGoblin.asset ScriptableObject ────
        var enemyData = AssetDatabase.LoadAssetAtPath<EnemyData>(enemyDataPath);
        if (enemyData == null)
        {
            Debug.LogError($"[SuicideGoblinWiring] EnemyData not found at {enemyDataPath}");
            return;
        }
        ...
        Debug.Log($"[SuicideGoblinWiring] SuicideGoblin.asset wired: modelPrefab={fbxAsset.name}, animatorController={controller.name}");
        Debug.Log("[SuicideGoblinWiring] Done! Suicide Goblin enemies will now use the custom model at runtime.");
    }
}
